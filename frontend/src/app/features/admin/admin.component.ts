...
interface ActiveUser { _id: string; name: string; email: string; }
  ...
  template: `
    <div class="page">
      ...
      <section class="card" style="margin-bottom:2rem;max-width:560px">
        <h2 style="font-size:1rem;font-weight:600;margin-bottom:1.25rem">Invite a New User</h2>
        ...
        <form [formGroup]="inviteForm" (ngSubmit)="sendInvite()">
          ...
          @if (inviteError()) {
            <p class="error-msg" style="margin-top:.75rem;margin-bottom:0">{{ inviteError() }}</p>
          }
        </form>

        <!-- Invite link shown after success -->
        @if (inviteLink()) {
          <div style="margin-top:1.25rem;padding:1rem;background:var(--bg);
                      border-radius:var(--radius);border:1px solid var(--border)">
            <p style="font-size:.8125rem;font-weight:500;margin-bottom:.5rem;color:var(--muted)">
              Share this registration link with the invitee:
            </p>
            <div style="display:flex;gap:.5rem;align-items:center">
              <code style="font-size:.8rem;word-break:break-all;flex:1;
                            background:var(--surface);padding:.375rem .625rem;
                            border-radius:4px;border:1px solid var(--border)">
                {{ inviteLink() }}
              </code>
              <button class="btn btn-outline btn-sm" style="flex-shrink:0" (click)="copyLink()">
                {{ copied() ? 'Copied!' : 'Copy' }}
              </button>
            </div>
          </div>
            ...
            @for (user of users(); track user._id) {
              <div class="card"
                ...
                <div style="flex:1;min-width:0">
                  ...
                  <div style="font-size:.8125rem;color:var(--muted)">{{ user.email }}</div>
                </div>
              </div>
            }
          </div>
...
export class AdminComponent implements OnInit {
  ...
  sendInvite() {
    ...
    this.http
      .post<{ inviteToken: string }>(`${environment.apiUrl}/auth/invite`, this.inviteForm.value)
      .subscribe({
        next: res => {
          ...
          // Refresh user list (new invite will appear once they register)
        },
        error: err => {
