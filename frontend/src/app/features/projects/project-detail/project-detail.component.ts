      ...
      @if (loading()) {
        ...
        <div style="margin-bottom:2rem">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem">
            <div>
              <a routerLink="/projects" class="btn btn-outline btn-sm"
                 style="margin-bottom:.875rem;display:inline-flex">← Projects</a>
              <h1 class="page-title">{{ project()!.name }}</h1>
              @if (project()!.description) {
                <p style="color:var(--muted);margin-top:.375rem">{{ project()!.description }}</p>
              }
            </div>
            @if (canManage()) {
        ...
        <section>
          ...
          @if (showAddMember()) {
            <div class="card" style="margin-bottom:.75rem">
              ...
              @if (addError()) {
                <p class="error-msg" style="margin-top:.75rem;margin-bottom:0">{{ addError() }}</p>
              }
            </div>
          }
            ...
            @for (member of project()!.members; track member._id) {
              <div class="card"
                ...
                <div>
                  ...
                  <span style="color:var(--muted);font-size:.875rem;margin-left:.5rem">{{ member.email }}</span>
                  @if (member._id === project()!.owner._id) {
                    <span class="badge badge-in-progress" style="margin-left:.625rem">Owner</span>
                  }
                </div>
                @if (canManage() && member._id !== project()!.owner._id) {
                  <button class="btn btn-danger btn-sm" (click)="removeMember(member._id)">Remove</button>
                }
              </div>
            }
          </div>
        ...
        <section style="margin-top:2.5rem">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.875rem">
            ...
            <button class="btn btn-primary btn-sm" (click)="openCreateTask('todo')">+ New Task</button>
          </div>
            ...
            <div class="board">
              ...
              <div class="board-col">
                <div class="board-col-header">
                  ...
                  <button class="btn btn-outline btn-sm"
                          ...
                          (click)="openCreateTask('todo')">+</button>
                </div>
              ...
              <div class="board-col">
                <div class="board-col-header">
                  ...
                  <span class="badge badge-in-progress">{{ inProgressTasks().length }}</span>
                  <button class="btn btn-outline btn-sm"
                          style="padding:.2rem .6rem"
                          (click)="openCreateTask('in-progress')">+</button>
                </div>
              ...
              <div class="board-col">
                <div class="board-col-header">
                  ...
                  <button class="btn btn-outline btn-sm"
                          style="padding:.2rem .6rem"
                          (click)="openCreateTask('done')">+</button>
                </div>
      ...
      <div class="card task-card" (click)="openEditTask(task)">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem">
          ...
          <button class="btn btn-danger btn-sm"
                  ...
                  (click)="deleteTask(task, $event)">×</button>
        </div>
        <div class="task-card-meta">
          <span class="badge badge-{{ task.priority }}">{{ task.priority }}</span>
          @if (task.assignee) {
            <span style="font-size:.75rem;color:var(--muted)">{{ task.assignee.name || task.assignee.email }}</span>
          }
          @if (task.dueDate) {
            <span style="font-size:.75rem;color:var(--muted)">{{ fmtDate(task.dueDate) }}</span>
          }
        </div>
      </div>
    </ng-template>
        ...
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <form [formGroup]="taskForm" (ngSubmit)="saveTask()">
            ...
            <div class="field">
              ...
              <textarea formControlName="description" placeholder="More details…"></textarea>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
              <div class="field">
                <label>Assignee <span style="color:var(--muted);font-weight:400">(optional)</span></label>
              ...
              <div class="field">
                <label>Due Date <span style="color:var(--muted);font-weight:400">(optional)</span></label>
...
export class ProjectDetailComponent implements OnInit {
  ...
  private loadTasks() {
    ...
    this.taskService.getAll(this.projectId).subscribe({
      next:  tasks => { this.tasks.set(tasks); this.taskLoading.set(false); },
  ...
  saveTask() {
    ...
    call.subscribe({
      next: saved => {
        if (existing) {
          this.tasks.update(list => list.map(t => t._id === saved._id ? saved : t));
        } else {
  ...
  deleteTask(task: Task, event: Event) {
    event.stopPropagation(); // prevent card click opening the edit modal
